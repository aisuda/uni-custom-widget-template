...
export default defineConfig(({ command, mode }) => {
  ...
  if (command === 'serve') {
    ...
    // command === 'build'
    const UNI_BUILD_MODE = process.env.UNI_BUILD_MODE || catchProcessParam('UNI_BUILD_MODE');
    const UNI_BUILD_LIB = process.env.UNI_BUILD_LIB || catchProcessParam('UNI_BUILD_LIB');
